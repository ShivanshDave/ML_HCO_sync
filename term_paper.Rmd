...
>By changing Omega, your system size.  Currently it looks like you have Omega=100 for the potassium channels, by default (line 89 of your code).
If you make Omega equal to 1e4 then the noise should be reduced by 90%, so to speak.  I tried this in your code, and the cells stopped firing.
This means that with current=60, a cell is in the "excitable" regime, where it will only fire if forced (by noise or by the other cell).  
